...
import com.it.hospital_manage.model.Schedule;
import com.it.hospital_manage.service.ApiService;
import com.it.hospital_manage.util.BeanUtils;
...
public class ApiServiceImpl implements ApiService {
    ...
    public boolean saveSchedule(String data) {
        ...
        for(int i=0, len=jsonArray.size(); i<len; i++) {
            ...
            schedule.setWorkTime(jsonObject.getString("workTime"));
    ...
    public static String getImgStr(String imgFile){
        byte[] data = getImagesBytes(imgFile);
        return new String(Base64.encodeBase64(data));
    }

    public static byte[] getImagesBytes(String imgUrl){
        URL url = null;
        InputStream is = null;
        ...
        try {
            ...
            while ((len = is.read(buffer))!=-1){
                // 用输出流往buffer里写入数据，中间参数代表从那个位置开始读，len代表读取的长度
